...
def plotdata(position, title):
	...
	pl.xlabel('Std. of Page Distribution')
	pl.ylabel('PDF')
	...
	pl.title('Avg Std Dev :' + format(std_clr_mean, '0.3f') + ' | ' + title)

	# Create a subplot for miss-rate bars
	mrate_pl = pl.subplot(pl_rows, pl_cols, position + pl_cols)
	x_axis = [1, 3]
	extrema = [(((float(net_alld_pages))/net_pages) * 100), (((float(net_rest_pages))/net_pages) * 100)]
	pl.bar(x_axis, extrema, width = 1)

	for i, j in zip(x_axis, extrema):
		mrate_pl.annotate('%0.1f %%' % (j), xy = (i, j), fontsize = pl_fontsize)
	...
	# Label the axis
	pl.xlabel('Pages in Allowed vs Restricted Colors')
	pl.ylabel('% of Pages')
...
def main():
	...
	if platform == 'tegra':
		pos = 1
		figname = 'Tegra_BW_Modf_CLRS_Scaled.png'

		# Place a title for this graph
		pl.suptitle('Tegra | BW-R | ' + part_size + ' | ' + ' Solo | Modf | Sets | Colors');
		
		for util in utilization:
			util = str(util)

			# Plot the data one set at a time
			do_set(platform, 'solo', 'modf', part, util, pos)
